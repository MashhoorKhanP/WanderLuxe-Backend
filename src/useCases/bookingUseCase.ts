...
class BookingUseCase {
  ...
  async payment(bookingData: IBooking) {
    if (bookingData) {
      const paymentData = await this.PaymentRepository.confirmPayment(
        bookingData
      );
      return {
        status: 200,
  ...
  async bookRoom(
    ...
    if (isWalletBalanceUsed === true) {
      ...
      const walletHistory = {
        ...
        transactionAmount:
          bookingDetails.paymentMethod === "Wallet Payment"
            ? bookingDetails.totalAmount
            : (user?.wallet as number),
        currentBalance:
          bookingDetails.paymentMethod === "Wallet Payment"
            ? (user?.wallet as number) - bookingDetails.totalAmount
            : 0,
      };
      ...
      const bookingData = {
        ...
        totalAmount:
          bookingDetails.paymentMethod === "Wallet Payment"
            ? bookingDetails.totalAmount
            : bookingDetails.totalAmount + user?.wallet,
    ...
    if (bookingDetails.appliedCouponId !== "No Coupon Applied") {
      const appliedCoupon = this.CouponRepository.findByIdAndUpdateCount(
        bookingDetails.appliedCouponId
      );
    }
    ...
    if (scheduledCheckOutDate <= today) {
      schedule.scheduleJob(scheduledCheckOutDate, async () => {
        ...
        const updatedRoomStatus =
          await this.RoomRepository.findByIdAndUpdateRoomStatus(
            // findby roomId and change status
  ...
  async getHotelBookings(hotelId: string) {
    const bookings = await this.BookingRepository.findBookingsByHotelId(
      hotelId
    );
    return {
      status: 200,
  ...
  async updateBooking(bookingId: string, reqBody: object) {
    ...
    const { status, userId, roomId } = reqBody as any;
    if (status === "Cancelled by Admin" || status === "Cancelled") {
      ...
      const walletHistory = {
        ...
        currentBalance: ((user?.wallet as number) +
          updatedBooking?.totalAmount) as any,
      };
      const updatedUser = await this.UserRepository.findByIdAndUpdateWallet(
        userId,
        walletHistory.transactionAmount,
